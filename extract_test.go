...
func TestExtractRunWithOptionalParams(t *testing.T) {
	...
	_, err := client.Extract.Run(context.TODO(), reductoai.ExtractRunParams{
		DocumentURL: reductoai.F("document_url"),
		Schema:      reductoai.F[any](map[string]interface{}{}),
		...
		ArrayExtract: reductoai.F(shared.ArrayExtractConfigParam{
			Enabled:                     reductoai.F(true),
			Mode:                        reductoai.F(shared.ArrayExtractConfigModeAuto),
			PagesPerSegment:             reductoai.F(int64(0)),
			StreamingExtractItemDensity: reductoai.F(int64(0)),
		}),
		...
		GenerateCitations: reductoai.F(true),
		...
		SystemPrompt: reductoai.F("system_prompt"),
	})
	if err != nil {
...
func TestExtractRunJobWithOptionalParams(t *testing.T) {
	...
	_, err := client.Extract.RunJob(context.TODO(), reductoai.ExtractRunJobParams{
		DocumentURL: reductoai.F("document_url"),
		Schema:      reductoai.F[any](map[string]interface{}{}),
		AdvancedOptions: reductoai.F(shared.AdvancedProcessingOptionsParam{
			AddPageMarkers:     reductoai.F(true),
